...
HRESULT DemoApp::CreateDeviceResources()
{
	...
	if (!m_pRenderTarget)
	{
		...
		if (SUCCEEDED(hr))
		{
			/* Создаю шрифт для обычных надписей */
			hr = m_pWriteFactory->CreateTextFormat(
				L"Algerian",
				NULL,
				DWRITE_FONT_WEIGHT_REGULAR,
...
/*ЭТА ШТУКА ОТСЛЕЖИВАЕТ ВСЕ СОБЫТИЯ, ПРОИСХОДЯЩИЕ С ОКНОМ, КАК ТО: СОЗДАНИЕ, ЗАКРЫТИЕ, РЕСАЙЗ, НАЖАТИЕ КЛАВИШ ИТД ИТП			*/	
...
LRESULT CALLBACK DemoApp::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		if (pDemoApp)
		{
			switch (message)
			{
				/* СМОТРИ, ВОВА, ЕТО ПЕРЕРИСОВКА, ТУТ МЫ РЕАГИРУЕМ НА ИЗМЕНЕНИЯ РАЗМЕРОВ ОКНА*/
				/*надо запилить макрос с максимальным размером окна и назвать его "TVOYA_MAMKA_SIZE" */
			case WM_SIZE:
			{
			...
			case WM_LBUTTONDOWN:
			{
				/*	это координаты точки, куда щелкнула мышка		*/
				float x, y;
				x = LOWORD(lParam);
				y = HIWORD(lParam);
			...
			/*адекватное закрытие окна, без него плохо, житья не будет*/
			/*очень информативные комментарии*/
			case WM_DESTROY:
			{
				PostQuitMessage(0);
			}
...
/*эта штука отвечает за перерисовку всея прилаженЪя*/
...
/*для упрощения работы и запоминания меньшего объема ебливых форматов ебливых функций
, я нарекаю вас, нет, не то, я создаю функцию, которая в БОЛЕЕ ПОНЯТНОМ ФОРМАТЕ
обеспечиваеи вывод текста на экран */
...
/*собсна, прочерочка на нажатие в прямогугольник, nothing else*/
...
void DemoApp::DrawPreparing(int x_center, int y_center, int width_of_cell)
{
	...
	/*Ректанглы корабликов для расстановки*/
